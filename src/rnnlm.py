...
class RNNLM(NNBase):
    ...
    def __init__(self, L0, U0=None,
        ...
        if U0 is not None:
            ...
            random.randn(self.vdim, self.hdim)*0.1
    ...
    def _acc_grads(self, xs, ys):
        ...
        deltas = zeros((ns, self.hdim))
        word_errors = zeros((ns, self.hdim))
        for t in range(ns-1, -1, -1):
            d1 = ps[t]
            d1[ys[t]] -= 1

            self.grads.U += outer(d1, hs[t])
            
            deltas[t] = self.params.U.T.dot(d1) * (hs[t] * (1 - hs[t]))

            self.sgrads.L[xs[t]] = deltas[t] - 0.0
            #word_errors[t] += deltas[t]
            self.grads.H += outer(deltas[t], hs[t-1])

            for bt in range(t-1, t - self.bptt-1, -1):
                if bt < 0:
                    break
                
                deltas[bt] = self.params.H.T.dot(deltas[bt+1]) * (hs[bt] * (1 - hs[bt]))
                  
                self.sgrads.L[xs[bt]] = deltas[bt] - 0.0
                #word_errors[bt] += deltas[bt]
                self.grads.H += outer(deltas[bt], hs[bt-1])

                

        #### END YOUR CODE ####
        #self.sparams.L[xs] = word_errors
        #for i in range(ns):
        #    self.sparams.L[xs[i]] = word_errors[i]
    ...
    def compute_loss(self, X, Y):
        ...
        Do not modify this function!
        """
        if not isinstance(X[0], ndarray) and not isinstance(X[0], Iterable): # single example
